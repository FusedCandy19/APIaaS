...
export async function dashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/dashboard/summary', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const logs30Days = await prisma.usageLog.findMany({
      where: {
        apiKey: { userId },
    ...
    const dailyMap = new Map<string, { date: string; requests: number; cost: number }>();
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    const modelBreakdownMap = new Map<string, { model: string; value: number; cost: number }>();
    for (const log of logs30Days) {
