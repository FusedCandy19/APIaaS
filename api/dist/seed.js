...
async function main() {
    ...
    // 3. Seed Users
    const adminPasswordHash = await bcryptjs_1.default.hash(config_1.config.ADMIN_PASSWORD, 10);
    const demoPasswordHash = await bcryptjs_1.default.hash('demo123', 10);
    const admin = await db_1.prisma.user.upsert({
        where: { email: config_1.config.ADMIN_EMAIL },
    ...
    const demoUser = await db_1.prisma.user.upsert({
    ...
    if (existingKeys.length === 0) {
        ...
        const prodDetails = crypto_1.cryptoHelper.generateKey('prod');
        const devDetails = crypto_1.cryptoHelper.generateKey('sandbox');
        const oldDetails = crypto_1.cryptoHelper.generateKey('old');
        const prodKey = await db_1.prisma.apiKey.create({
            data: {
                userId: demoUser.id,
        ...
        const devKey = await db_1.prisma.apiKey.create({
            data: {
                userId: demoUser.id,
        ...
        await db_1.prisma.apiKey.create({
            data: {
                userId: demoUser.id,
        ...
        for (let day = 0; day < 7; day++) {
            ...
            for (let req = 0; req < numRequests; req++) {
                const selectedModel = modelIds[Math.floor(Math.random() * modelIds.length)];
